...
The dedicated release bucket must be configured as public read in Bitiful. The release workflow uploads every release artifact with public-read object access, then uses `s3api head-object` to read each object back and verify its byte size. It generates `picmark/update.json` only after that verification; this mirror manifest contains direct domestic URLs for the setup and portable packages. The workflow also requires the public mirror manifest to return HTTP 200 before creating the GitHub Release.
